...
namespace ZeroLogic::Search{
    ...
    template <bool white>
    void mm(Move m, Gamestate* gs, MoveList* ml){
        gs->makemove<!white>(m);
...
#define move_macro gs.makemove<!white>(m); if (!d || clear_since > 1) {overall_nodes++; Eval e(&gs); return ((white) ? 1 : -1) * e.get();} MoveList ml; MoveGenerator<white> mg(&gs, &ml); mg.go(); if (ml.list[0] == checkmate) return mate_in<white>(d); else if (ml.list[0] == stalemate) return draw;
#define minmax(move) if (v >= b) { return b; } if (v > a) { a = v; higher_pv.put_in(move); *lower_pv = higher_pv; }

    template<bool white>
    constexpr eval mate_in(int depth){
        eval val = mate - (full_depth - depth);
        if (white) return -val;
        else       return val;
    }
    constexpr int mate_in(eval val){
        return val/abs(val) * (mate - abs(val));
    }
    ...
    eval see(Gamestate gs, Move m, pv* lower_pv, eval a, eval b, int actual_depth, int exchange_index){
        ...
        for (int i = 0; i < ml.index_front; ++i){
            ...
            else v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, 0, 0, actual_depth + 1);
            minmax(ml.list[i])
        }
        for (int i = ml.index_back + 1; i < 218; ++i){
            if (Time::timed_out()) return 0;
            pv higher_pv{};
            eval v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, 0, 0, actual_depth + 1);
            minmax(ml.list[i])
        }
        return a;
    }
    ...
    template <bool white>
    eval best_move(Gamestate gs, Move m, pv* lower_pv, eval a, eval b, int d, int clear_since, int actual_depth){
        if (actual_depth > max_depth) max_depth = actual_depth;
        move_macro
        for (int i = 0; i < ml.index_front; ++i){
            if (Time::timed_out()) return 0;
            ...
            else v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, d - 1, 0, actual_depth + 1);
            minmax(ml.list[i])
        }
        for (int i = ml.index_back + 1; i < 218; ++i){
            if (Time::timed_out()) return 0;
            pv higher_pv{};
            eval v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, d - 1, clear_since + 1, actual_depth + 1);
            minmax(ml.list[i])
        }
        return a;
    }
    ...
    eval pv_search(Gamestate gs, Move m, pv* lower_pv, pv* old_pv, eval a, eval b, int d, int actual_depth){
        ...
        if (ml.list[0] == checkmate) return mate_in<white>(d);
        ...
        Move pv_move = old_pv->get_out();
        {
            ...
            else               v = -best_move<!white>(gs, pv_move, &higher_pv, -b, -a, 1, 0, actual_depth + 1);
            minmax(pv_move)
        }
        for (int i = 0; i < ml.index_front; ++i){
            if (Time::timed_out()) return 0;
        ...
        for (int i = ml.index_back + 1; i < 218; ++i){
            ...
            if (ml.list[i] == pv_move) continue;
            pv higher_pv{};
            eval v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, (d > 0) ? d - 1 : 0, 1, actual_depth + 1);
            minmax(ml.list[i])
        }
        return a;
    }
    ...
    void start_iterative_deepening(Gamestate gs, int msec){
        ...
        Move best_move_var;
        eval low_window = 30, high_window = 30, current_eval, last_eval = 0;
        int next_depth = 1;
        ...
        while (true){
            ...
            current_eval = [&]() -> eval {
                ...
                MoveGenerator<white> mg(&gs, &ml);
                mg.go();
                if (ml.list[0] == checkmate) return mate_in<white>(next_depth);
                else if (ml.list[0] == stalemate) return draw;
                Move pv_move = 0xffff;
                if (curr_pv.index) {
                    ...
                    else v = -best_move<!white>(gs, pv_move, &higher_pv, -b, -a, (next_depth) ? next_depth - 1 : 1, 0, 1);
                    minmax(pv_move)
                }
                for (int i = 0; i < ml.index_front; ++i) {
                    if (Time::timed_out()) return 0;
                    if (ml.list[i] == pv_move) continue;
                    pv higher_pv{};
                    eval v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, next_depth, 0, 1);
                    minmax(ml.list[i])
                }
                for (int i = ml.index_back + 1; i < 218; ++i) {
                    if (Time::timed_out()) return 0;
                    if (ml.list[i] == pv_move) continue;
                    pv higher_pv{};
                    eval v = -best_move<!white>(gs, ml.list[i], &higher_pv, -b, -a, next_depth - 1, 1, 1);
                    minmax(ml.list[i])
                }
                return a;
            }();
            ...
            if (lower_pv && lower_pv->index){
                ss << " pv";
                for (int i = lower_pv->index - 1; i; i--) ss << " " << Misc::numToString(lower_pv->list[i]);
            }
            ...
            else if (abs(current_eval) > 30000){best_move_var = lower_pv->list[lower_pv->index - 1]; break;}
            else{
                next_depth++;
